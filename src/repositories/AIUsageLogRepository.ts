...
export class D1AIUsageLogRepository implements AIUsageLogRepository {
  ...
  async create(data: CreateAIUsageLogInput): Promise<AIUsageLog> {
    ...
    const log: AIUsageLog = {
      ...
      prompt_length: (data as any).input_size || data.prompt_length || 0,
      max_tokens: data.max_tokens || 0,
      tokens_used: data.tokens_used,
      response_length: (data as any).output_size || data.response_length || 0,
    ...
    await this.db
      ...
      .bind(
        ...
        log.model,
        log.use_case,
        log.prompt_length || null,
        log.max_tokens || null,
        log.tokens_used,
        log.response_length || null,
        log.duration_ms,
        log.cost_usd,
        log.user_id || null,
